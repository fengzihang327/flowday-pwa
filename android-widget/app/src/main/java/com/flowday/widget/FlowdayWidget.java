    ...
    private static void updateWidget(Context context, AppWidgetManager mgr, int widgetId) {
        ...
        try {
            ...
            views.setTextViewText(R.id.w_msg, total == 0 ? "暂无任务" :
                    pct == 100 ? "全部完成" : (total - done) + " 项待完成");

            // Task rows
            int[] dotIds = {R.id.w_dot1, R.id.w_dot2, R.id.w_dot3};
            int[] labelIds = {R.id.w_label1, R.id.w_label2, R.id.w_label3};
            int[] timeIds = {R.id.w_time1, R.id.w_time2, R.id.w_time3};
            ...
            if (tasks != null && tasks.length() > 0) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    views.setTextViewText(dotIds[i], isDone ? "✓" : "");
                    views.setTextViewText(labelIds[i], title);
                    views.setTextViewText(timeIds[i], timeStr);
                    views.setInt(dotIds[i], "setBackgroundResource",
                            isDone ? R.drawable.dot_done : R.drawable.dot_open);
                ...
                views.setInt(R.id.w_more, "setVisibility",
                        tasks.length() > 3 ? android.view.View.VISIBLE : android.view.View.GONE);
                if (tasks.length() > 3) {
                    views.setTextViewText(R.id.w_more, "+" + (tasks.length() - 3) + " 项");
                }
